...
internal class TreasureMapManager
{
    ...
    private readonly DataValue<uint> _clearCount;
    private readonly DataValue<byte> _mapCount;
    private readonly TreasureMapData[] _mapData = new TreasureMapData[99];
    ...
    public ushort ClearCount
    {
        ...
        set => _clearCount.Value = (uint)(((int)_clearCount.Value & -1048513) | ((value << 6) & 1048512));
    }
    ...
    public void MoveTo(int srcIndex, int toIndex)
    {
        if (srcIndex >= _mapCount.Value || toIndex >= _mapCount.Value || srcIndex == toIndex)
            return;
        var bytesData = _mapData[srcIndex].GetBytesData();
        if (srcIndex > toIndex)
