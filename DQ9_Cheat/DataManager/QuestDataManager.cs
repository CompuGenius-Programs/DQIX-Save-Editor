...
namespace DQ9_Cheat.DataManager
{
  internal class QuestDataManager
  {
    ...
    private QuestMatterData[] _questMatterData;

    public QuestDataManager(SaveData owner)
    {
      _clearCount = new DataValue<byte>(owner, 16035U, null, 0, byte.MaxValue);
      _orderCount = new DataValue<byte>(owner, 12404U, null, 0, 8);
      for (int index = 0; index < 8; ++index)
        _orderInfomation[index] = new DataValue<ushort>(owner, (uint) (12408 + 16 * index), null, 0, ushort.MaxValue);
    ...
    public DataValue<byte> ClearCount => _clearCount;
    ...
    {
      byte num = 0;
      foreach (QuestMatterData questMatterData in _questMatterData)
      {
        if (questMatterData.State == QuestState.Clear)
          ++num;
      }
      _clearCount.Value = num;
    }
    ...
    public void ReviseOrderCount()
    {
      ...
      for (int index1 = 0; index1 < _orderCount.Value; ++index1)
      {
        byte num = (byte) (_orderInfomation[index1].Value & (uint) byte.MaxValue);
        bool flag = false;
      ...
      byteList1.Clear();
      for (int index = 0; index < _questMatterData.Length && _orderCount.Value < 8; ++index)
      {
        if (_questMatterData[index].State == QuestState.Order && !byteList2.Contains(QuestDataList.List[index].DataIndex))
    ...
    private void AddOrderInformation(int dataIndex)
    {
      if (_orderCount.Value >= 8)
        return;
      for (int index = 0; index < _orderCount.Value; ++index)
      {
    ...
    private void RemoveOrderInfomation(int dataIndex)
    {
      for (int index1 = 0; index1 < _orderCount.Value; ++index1)
      {
        ...
        {
          for (int index2 = index1; index2 < _orderCount.Value; ++index2)
            _orderInfomation[index2].Value = index2 != _orderCount.Value - 1 ? _orderInfomation[index2 + 1].Value : (ushort) 0;
          --_orderCount.Value;
        }
      }
    }
    ...
    private class QuestMatterData
    {
      ...
      public QuestState State
      {
        get
        {
          QuestElement questElement = QuestDataList.List[_index];
          byte num = (questElement.DataIndex & 1) != 0 ? (byte) ((uint) _state.Value >> 4) : (byte) (_state.Value & 15U);
          InnerQuestState innerQuestState = (InnerQuestState) (num & 7);
          if (questElement.AdditionalQuest)
          {
      ...
      {
        QuestElement questElement = QuestDataList.List[_index];
        _state.Value = (questElement.DataIndex & 1) != 0 ? (byte) (_state.Value & 15 | 32 | (questElement.AdditionalQuest ? 128 : 0)) : (byte) (_state.Value & 240 | 2 | (questElement.AdditionalQuest ? 8 : 0));
        _clearFlag.Value &= (byte) ~(1 << questElement.DataIndex % 8);
      }
      ...
      {
        QuestElement questElement = QuestDataList.List[_index];
        _state.Value = (questElement.DataIndex & 1) != 0 ? (byte) (_state.Value & 15 | 48 | (questElement.AdditionalQuest ? 128 : 0)) : (byte) (_state.Value & 240 | 3 | (questElement.AdditionalQuest ? 8 : 0));
        _clearFlag.Value = (byte) (_clearFlag.Value & ~(1 << questElement.DataIndex % 8) | 1 << questElement.DataIndex % 8);
      }
      ...
      internal void SetRecieve()
      {
        ...
        if (!questElement.AdditionalQuest)
          return;
        _state.Value = (questElement.DataIndex & 1) != 0 ? (byte) (_state.Value & 15 | 128) : (byte) (_state.Value & 240 | 8);
        _clearFlag.Value &= (byte) ~(1 << questElement.DataIndex % 8);
      }
      ...
      private bool ClearFlag
      {
        get
        {
          return (_clearFlag.Value & 1 << QuestDataList.List[_index].DataIndex % 8) != 0;
        }
      }
      ...
      public ushort Year
      {
        ...
        set
        {
          _dateTime.Value = (uint) ((int) _dateTime.Value & sbyte.MinValue | value - 2000 & sbyte.MaxValue);
        }
      }
      ...
      public byte Month
      {
        ...
        set
        {
          _dateTime.Value = (uint) ((int) _dateTime.Value & -1921 | value << 7 & 1920);
        }
      }
      ...
      public byte Day
      {
        ...
        set
        {
          _dateTime.Value = (uint) ((int) _dateTime.Value & -63489 | value << 11 & 63488);
        }
      }
      ...
      public byte Hour
      {
        ...
        set
        {
          _dateTime.Value = (uint) ((int) _dateTime.Value & -2031617 | value << 16 & 2031616);
        }
      }
      ...
      public byte Minute
      {
        ...
        set
        {
          _dateTime.Value = (uint) ((int) _dateTime.Value & -132120577 | value << 21 & 132120576);
        }
      }
