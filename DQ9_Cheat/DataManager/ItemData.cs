...
internal class ItemData
{
    ...
    private readonly DataValue<ushort>[] _accessory = new DataValue<ushort>[64];
    private readonly DataValue<byte>[] _accessoryNum = new DataValue<byte>[64];
    private readonly DataValue<ushort>[] _arm = new DataValue<ushort>[80];
    private readonly DataValue<byte>[] _armNum = new DataValue<byte>[80];
    private readonly DataValue<ushort>[] _bag = new DataValue<ushort>[152];
    private readonly DataValue<byte>[] _bagNum = new DataValue<byte>[152];
    private readonly DataValue<ushort>[] _head = new DataValue<ushort>[144];
    private readonly DataValue<byte>[] _headNum = new DataValue<byte>[144];
    private readonly DataValue<ushort>[] _important = new DataValue<ushort>[94];
    private readonly DataValue<byte>[] _importantNum = new DataValue<byte>[94];
    private readonly DataValue<ushort>[] _lowerBody = new DataValue<ushort>[96];
    private readonly DataValue<byte>[] _lowerBodyNum = new DataValue<byte>[96];
    private readonly DataValue<ushort>[] _shield = new DataValue<ushort>[48];
    private readonly DataValue<byte>[] _shieldNum = new DataValue<byte>[48];
    private readonly DataValue<ushort>[] _shoe = new DataValue<ushort>[112];
    private readonly DataValue<byte>[] _shoeNum = new DataValue<byte>[112];
    private readonly DataValue<ushort>[,] _tools = new DataValue<ushort>[4, 8];
    private readonly DataValue<ushort>[] _upperBody = new DataValue<ushort>[192];
    private readonly DataValue<byte>[] _upperBodyNum = new DataValue<byte>[192];
    private readonly DataValue<ushort>[] _weapon = new DataValue<ushort>[272];
    private readonly DataValue<byte>[] _weaponNum = new DataValue<byte>[272];

    public ItemData(SaveData owner)
    {
        for (var index1 = 0; index1 < 4; ++index1)
        for (var index2 = 0; index2 < 8; ++index2)
            _tools[index1, index2] =
                new DataValue<ushort>(owner, (uint)(7578 + 18 * index1 + 2 * index2), null, 0, ushort.MaxValue);
        ...
        for (var index = 0; index < 272; ++index)
        {
    ...
    {
        return partyIndex < 4 && toolIndex >= 0 && toolIndex < 8
            ? ItemDataList.GetItem(ItemType.Tool, _tools[partyIndex, toolIndex].Value)
            : null;
    }
    ...
    {
        var itemDataList = GetItemDataList(itemType);
        return index >= 0 && index < itemDataList.Length
            ? ItemDataList.GetItem(itemType, itemDataList[index].Value)
            : null;
    }
    ...
    {
        var itemNumList = GetItemNumList(itemType);
    ...
    public void PossessAll(ItemType itemType, byte count)
    {
        var itemDataList = GetItemDataList(itemType);
        var itemNumList = GetItemNumList(itemType);
        var list = ItemDataList.GetList(itemType, null);
        for (var index = 0; index < GetItemMaxCount(itemType); ++index)
    ...
    public int RemoveOverlap(ItemType itemType)
    {
        ...
        var itemNumList = GetItemNumList(itemType);
        var num = 0;
        for (var index1 = 0; index1 < GetItemMaxCount(itemType) - 1; ++index1)
            if (itemDataList[index1].Value != ushort.MaxValue)
                for (var index2 = index1 + 1; index2 < GetItemMaxCount(itemType); ++index2)
    ...
    public void SqueezeGap(ItemType itemType)
    {
        ...
        var itemDataList = GetItemDataList(itemType);
        var itemNumList = GetItemNumList(itemType);
        for (var index2 = 0; index2 < GetItemMaxCount(itemType) - 1; ++index2)
            if (itemDataList[index2].Value != ushort.MaxValue)
            {
                if (index1 != -1)
                {
