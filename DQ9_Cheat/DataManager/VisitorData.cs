...
internal class VisitorData
{
    ...
    private readonly DataValue<uint> _alchemyCount;
    ...
    private readonly DataValue<byte> _fashionCompRate;
    private readonly DataValue<ushort> _handle;
    private readonly DataValue<ushort> _handle2;
    private readonly DataValue<byte> _haveTreasureMap;
    private readonly uint _index;
    private readonly DataValue<ushort> _itemCompRate;
    private readonly DataValue<byte> _jobAndPlace;
    private readonly DataValue<byte> _level;
    private readonly DataValue<ushort> _lodgingDayMonthDay;
    private readonly DataValue<byte> _lodgingDayYear;
    private readonly DataValue<ushort> _monsterCompRate;
    private readonly DataValue<ushort> _multiPlayTimeHour;
    private readonly DataValue<ushort> _playTimeHour;
    private readonly DataValue<ushort> _playTimeMinute;
    ...
    private readonly DataValue<uint> _questVisitorAlchemy;
    private readonly DataValue<uint> _seq;
    private readonly DataValue<byte> _sex;
    private readonly DataValue<byte> _skinEyeColorSex;
    private readonly DataValue<ushort> _titleCount;
    private readonly DataValue<byte> _transmigration;
    private readonly DataValue<ushort> _treasureMapCount;
    private readonly DataValue<ulong> _uid;
    private readonly DataValue<uint> _victoryCount;
    private readonly DataValue<ushort> _visible;

    public VisitorData(SaveData owner, uint index)
    {
        _index = index;
        var offset = (uint)(16200 + 232 * (int)index);
        ...
        Name = new DataValueString(owner, offset, null, 10, true);
        ...
        ProfileMessage = new DataValueString(owner, 116U + offset, null, 114, false, fillData);
        ...
        FigureWidth = new DataValue<ushort>(owner, 50U + offset, null, 0, ushort.MaxValue);
        FigureHeight = new DataValue<ushort>(owner, 52U + offset, null, 0, ushort.MaxValue);
        HairType = new DataValue<byte>(owner, 32U + offset, null, 0, byte.MaxValue);
        HairColor = new DataValue<byte>(owner, 47U + offset, null, 0, byte.MaxValue);
        FaceType = new DataValue<byte>(owner, 30U + offset, null, 0, byte.MaxValue);
        ...
        TreasureMapData = new TreasureMapData(owner, 80U + offset);
    }

    internal DataValueString Name { get; }
    ...
    public bool HaveTreasureMap
    {
        ...
        set => _haveTreasureMap.Value = (byte)((_haveTreasureMap.Value & sbyte.MaxValue) | (value ? 128 : 0));
    }

    public ushort LodgingDay_Year
    {
        get => (ushort)(2000 + (_lodgingDayYear.Value & sbyte.MaxValue));
        set => _lodgingDayYear.Value = (byte)((_lodgingDayYear.Value & 128) | ((value - 2000) & sbyte.MaxValue));
    }

    public byte LodgingDay_Month
    {
        get => (byte)((_lodgingDayMonthDay.Value & 1920) >> 7);
        set => _lodgingDayMonthDay.Value = (ushort)((_lodgingDayMonthDay.Value & 63615) | ((value << 7) & 1920));
    }

    public byte LodgingDay_Day
    {
        get => (byte)((_lodgingDayMonthDay.Value & 63488) >> 11);
        set => _lodgingDayMonthDay.Value = (ushort)((_lodgingDayMonthDay.Value & 2047) | ((value << 11) & 63488));
    }

    public byte Level
    {
        get => (byte)(_level.Value & (uint)sbyte.MaxValue);
        set => _level.Value = (byte)((_level.Value & 128) | (value & sbyte.MaxValue));
    }

    public byte Transmigration
    {
        get => (byte)((_transmigration.Value & 30) >> 1);
        set => _transmigration.Value = (byte)((_transmigration.Value & 225) | ((value << 1) & 30));
    }
    ...
    public ushort ProfileBirthYear
    {
        ...
        set => _profileBirthYearMonth.Value = (ushort)((_profileBirthYearMonth.Value & 61440) | (value & 4095));
    }

    public byte ProfileBirthMonth
    {
        get => (byte)((uint)_profileBirthYearMonth.Value >> 12);
        set => _profileBirthYearMonth.Value = (ushort)((_profileBirthYearMonth.Value & 4095) | (value << 12));
    }
    ...
    public uint VictoryCount
    {
        ...
        set => _victoryCount.Value = (uint)(((int)_victoryCount.Value & -8388545) | (((int)value << 6) & 8388544));
    }

    public byte MonsterCompRate
    {
        get => (byte)((_monsterCompRate.Value & 8128) >> 6);
        set => _monsterCompRate.Value = (ushort)((_monsterCompRate.Value & 57407) | ((value << 6) & 8128));
    }

    public byte ItemCompRate
    {
        get => (byte)((_itemCompRate.Value & 4064) >> 5);
        set => _itemCompRate.Value = (ushort)((_itemCompRate.Value & 61471) | ((value << 5) & 4064));
    }

    public byte FashionCompRate
    {
        get => (byte)(_fashionCompRate.Value & (uint)sbyte.MaxValue);
        set => _fashionCompRate.Value = (byte)((_itemCompRate.Value & 128) | (value & sbyte.MaxValue));
    }

    public ushort QuestClearCount
    {
        get => (ushort)(_questVisitorAlchemy.Value & 511U);
        set => _questVisitorAlchemy.Value = (uint)(((int)_questVisitorAlchemy.Value & -512) | (value & 511));
    }

    public ushort VisitorCount
    {
        get => (ushort)((_questVisitorAlchemy.Value & 8388096U) >> 9);
        set => _questVisitorAlchemy.Value =
            (uint)(((int)_questVisitorAlchemy.Value & -8388097) | ((value << 9) & 8388096));
    }
    ...
    public byte AlchemyCompRate
    {
        ...
        set => _questVisitorAlchemy.Value =
            (uint)(((int)_questVisitorAlchemy.Value & -1065353217) | ((value << 23) & 1065353216));
    }

    public ushort TitleCount
    {
        get => (ushort)(_titleCount.Value & 1023U);
        set => _titleCount.Value = (ushort)((_titleCount.Value & 64512) | (value & 1023));
    }

    public ushort TreasureMapCount
    {
        get => (ushort)((_treasureMapCount.Value & 65532) >> 2);
        set => _treasureMapCount.Value = (ushort)((_treasureMapCount.Value & 3) | (value << 2));
    }

    public ushort AlchemyCount
    {
        get => (ushort)((_alchemyCount.Value & 4194240U) >> 6);
        set => _alchemyCount.Value = (uint)(((int)_alchemyCount.Value & -4194241) | ((value << 6) & 4194240));
    }

    public byte Job
    {
        get => (byte)(_jobAndPlace.Value & 15U);
        set => _jobAndPlace.Value = (byte)((_jobAndPlace.Value & 240) | (value & 15));
    }
    ...
    public byte Place
    {
        get
        {
            var num = (byte)((_jobAndPlace.Value & 240) >> 4);
    ...
    public ushort PlayTimeHour
    {
        ...
        set => _playTimeHour.Value = (ushort)((_playTimeHour.Value & 49152) | (value & 16383));
    }

    public byte PlayTimeMinute
    {
        get => (byte)(_playTimeMinute.Value & (uint)sbyte.MaxValue);
        set => _playTimeMinute.Value = (ushort)((_playTimeMinute.Value & 65408) | (value & sbyte.MaxValue));
    }

    public byte MultiPlayTimeMinute
    {
        get => (byte)((_playTimeMinute.Value & 16256) >> 7);
        set => _playTimeMinute.Value = (ushort)((_playTimeMinute.Value & 49279) | ((value << 7) & 16256));
    }

    public ushort MultiPlayTimeHour
    {
        get => (ushort)(_multiPlayTimeHour.Value & 16383U);
        set => _multiPlayTimeHour.Value = (ushort)((_multiPlayTimeHour.Value & 49152) | (value & 16383));
    }
    ...
    public byte CharaSex
    {
        ...
        set => _skinEyeColorSex.Value = (byte)((_skinEyeColorSex.Value & 254) | (value & 1));
    }

    public byte SkinColor
    {
        get => (byte)((_skinEyeColorSex.Value & 14) >> 1);
        set => _skinEyeColorSex.Value = (byte)((_skinEyeColorSex.Value & 241) | (value << 1));
    }

    public byte EyeColor
    {
        get => (byte)((_skinEyeColorSex.Value & 240) >> 4);
        set => _skinEyeColorSex.Value = (byte)((_skinEyeColorSex.Value & 15) | (value << 4));
    }
    ...
    public TreasureMapData TreasureMapData { get; }
    ...
    public ulong UID
    {
        ...
        set => _uid.Value = (ulong)(((long)_uid.Value & -281474976710656L) | ((long)value & 281474976710655L));
    }
    ...
    {
        TreasureMapData.CalculateDetail();
    }
    ...
    {
        var destinationArray = new byte[232];
        Array.Copy(SaveDataManager.Instance.SaveData.Data, (uint)(16200 + 232 * (int)_index), destinationArray, 0L,
            232L);
