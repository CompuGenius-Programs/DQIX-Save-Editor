...
{
    private readonly DataValue<byte> _grade;
    private readonly DataValue<uint> _historyCharacter;
    private readonly DataValue<ulong> _uid;
    private readonly DataValue<ushort> _visitorCount;
    private readonly DataValue<uint> _visitorRealCount;

    public RikkaData(SaveData owner)
    {
        var rikkaInnDataControl = MainWindow.Instance.RikkaInnDataControl;
        rikkaInnDataControl.BeginUpdate();
        VisitorManager = new VisitorManager(owner);
    ...
    {
        return index >= 0 && index < HistoryCharacterList.List.Count &&
               (_historyCharacter.Value & (1 << HistoryCharacterList.List[index].Index)) != 0L;
    }
    ...
    public void SetHistoryCharacter(int index, bool value)
    {
        if (index < 0 || index >= HistoryCharacterList.List.Count)
            return;
        var index1 = HistoryCharacterList.List[index].Index;
