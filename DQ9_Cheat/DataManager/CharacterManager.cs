...
internal class CharacterManager
{
    ...
    private readonly DataValue<byte> _characterStandbyCount;
    private readonly DataValue<byte> _partyCharacterCount;
    private readonly DataValue<byte>[] _partyOrder = new DataValue<byte>[4];
    ...
    public CharacterManager(SaveData owner)
    {
        ...
        for (uint index = 0; index < 13U; ++index)
            CharacterData[(int)index] = new CharacterData(owner, index);
    ...
    public CharacterData[] CharacterData { get; } = new CharacterData[13];

    public byte PartyCharacterCount
    {
        get => (byte)(_partyCharacterCount.Value & 7U);
        set => _partyCharacterCount.Value = (byte)((_partyCharacterCount.Value & 248) | (value & 7));
    }

    public byte CharacterStandbyCount
    {
        get => (byte)(_characterStandbyCount.Value & 15U);
        set => _characterStandbyCount.Value = (byte)((_characterStandbyCount.Value & 240) | (value & 15));
    }

    public byte CharacterCount => (byte)(PartyCharacterCount + (uint)CharacterStandbyCount);

    public void OnLoaded()
    {
        for (var index = 0; index < _partyCharacterCount.Value; ++index)
    ...
    public void MoveTo(int srcIndex, int toIndex)
    {
        if (srcIndex >= CharacterCount || toIndex >= CharacterCount || srcIndex == toIndex)
            return;
        var bytesData = CharacterData[srcIndex].GetBytesData();
        if (srcIndex > toIndex)
            for (var index = srcIndex; index > toIndex; --index)
                CharacterData[index].Copy(CharacterData[index - 1].GetBytesData());
        else if (srcIndex < toIndex)
            for (var index = srcIndex; index < toIndex; ++index)
                CharacterData[index].Copy(CharacterData[index + 1].GetBytesData());
        CharacterData[toIndex].Copy(bytesData);
    ...
    public void DeleteCharacter(int index)
    {
        ...
        SaveDataManager.Instance.UndoRedoMgr.BeginPluralEdit();
        MoveTo(index, CharacterCount - 1);
        CharacterData[CharacterCount - 1].Clear();
        --_characterStandbyCount.Value;
        SaveDataManager.Instance.UndoRedoMgr.EndPluralEdit();
    }
    ...
    public bool CreateCharacter()
    {
        ...
        ++_characterStandbyCount.Value;
        CharacterData[CharacterCount - 1].Clear();
        CharacterData[CharacterCount - 1].InitNewChara();
