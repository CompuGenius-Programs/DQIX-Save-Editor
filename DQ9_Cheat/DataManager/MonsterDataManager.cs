  ...
  internal class MonsterDataManager
  {
    ...
    public MonsterDataManager(SaveData owner)
    {
      for (uint index = 0; index < 307U; ++index)
        _monsterData[(int) index] = new DataValue<uint>(owner, (uint) (13784 + (int) index * 4), null, 0U, uint.MaxValue);
    ...
    {
      ushort num = (ushort) (_monsterData[index].Value & 1023U);
    ...
    public bool IsFindOut(int index) => ((int) _monsterData[index].Value & 1024) != 0;
    ...
    {
      byte num = (byte) ((_monsterData[index].Value & 260096U) >> 11);
    ...
    {
      byte num = (byte) ((_monsterData[index].Value & 33292288U) >> 18);
      return num <= 99 ? num : (byte) 99;
    }
