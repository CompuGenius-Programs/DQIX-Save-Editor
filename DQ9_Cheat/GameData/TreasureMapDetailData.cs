  ...
  internal class TreasureMapDetailData
  {
    ...
    public TreasureMapDetailData()
    {
      ...
      for (int index = 0; index < 16; ++index)
        _treasureBoxInfoList[index] = new List<TreasureBoxInfo>();
    ...
    public bool IsValidSeed => _validSeed;

    public bool IsValidRank => _validRankList.Contains(_mapRank);

    public bool IsValidPlace(byte place) => _validPlaceList.Contains(place);
    ...
    {
      get => _lowRankValidPlaceList.AsReadOnly();
    }

    public ReadOnlyCollection<byte> MiddleRankValidPlaceList
    {
      get => _middleRankValidPlaceList.AsReadOnly();
    }

    public ReadOnlyCollection<byte> HighRankValidPlaceList
    {
      get => _highRankValidPlaceList.AsReadOnly();
    }
    ...
    public string MapName
    {
      get
      {
        if (MapRank < 2 || MapRank > 248)
          return "Unknown";
        return string.Format("{0} {2} of {1} Lv. {3}", TreasureMapDataTable.TreasureMapName1_Table[_details[5] - 1], TreasureMapDataTable.TreasureMapName2_Table[_details[6] - 1], TreasureMapDataTable.TreasureMapName3_Table[_name3Index], _details[4]);
      }
    }
    ...
    public string MapName1
    {
      get
      {
        return MapRank < 2 || MapRank > 248 ? "Unknown" : TreasureMapDataTable.TreasureMapName1_Table[_details[5] - 1];
      }
    }
    ...
    public string MapName2
    {
      get
      {
        return MapRank < 2 || MapRank > 248 ? "Unknown" : TreasureMapDataTable.TreasureMapName2_Table[_details[6] - 1];
      }
    }
    ...
    public string MapName3
    {
      get
      {
        return MapRank < 2 || MapRank > 248 ? "Unknown" : TreasureMapDataTable.TreasureMapName3_Table[_name3Index];
      }
    }

    public byte MapName3Index => _name3Index;

    public int MapLevel
    {
      get => MapRank < 2 || MapRank > 248 ? 0 : _details[4];
    }
    ...
    private uint Seek4(byte[] table1, byte[] table2, int roopCount)
    {
      for (int index1 = 0; index1 < roopCount; ++index1)
      {
        if (MapRank >= table1[index1 * 4] && MapRank <= table1[index1 * 4 + 1])
        {
          ...
          int num2 = (int) (GenerateRandom() % num1);
          int num3 = 0;
    ...
    private unsafe void GenerateFloorMap(int floor, uint address)
    {
      ...
      int num1 = (_dungeonInfo[floor, index1 + 2] - _dungeonInfo[floor, index1] + 1) / 2;
      int num2 = (_dungeonInfo[floor, index1 + 3] - _dungeonInfo[floor, index1 + 1] + 1) / 2;
      int num3 = 0;
      ...
      for (int index3 = _dungeonInfo[floor, index1]; index3 <= _dungeonInfo[floor, index1 + 2]; ++index3)
      {
        ...
        switch (num8)
        {
          ...
          default:
              ...
              if (num8 != 8)
              {
                switch (_dungeonInfo[floor, index3 + (_dungeonInfo[floor, index1 + 1] - 1 << 4) + 792])
                {
                  ...
                  case 8:
                    continue;
                  default:
                    int random1 = (int) GenerateRandom(0U, (uint) num2);
                    int num9 = index3 - 1;
                    for (int index4 = 0; index4 < random1 && _dungeonInfo[floor, index3 + (_dungeonInfo[floor, index1 + 1] + index4 << 4) + 792] != 8 && _dungeonInfo[floor, index3 + (_dungeonInfo[floor, index1 + 1] + index4 + 1 << 4) + 792] != 1 && (_dungeonInfo[floor, (_dungeonInfo[floor, index1 + 1] + index4 << 4) + index3 + 1 + 792] == 1 || _dungeonInfo[floor, (_dungeonInfo[floor, index1 + 1] + index4 + 1 << 4) + index3 + 1 + 792] != 1) && (_dungeonInfo[floor, (_dungeonInfo[floor, index1 + 1] + index4 << 4) + num9 + 792] == 1 || _dungeonInfo[floor, num9 + (_dungeonInfo[floor, index1 + 1] + index4 + 1 << 4) + 792] != 1); ++index4)
                      _dungeonInfo[floor, index3 + (_dungeonInfo[floor, index1 + 1] + index4 << 4) + 792] = 1;
                    continue;
                }
              }
            ...
            int random2 = (int) GenerateRandom(0U, (uint) num7);
            for (int index5 = 0; index5 < random2; ++index5)
            {
              int index6 = index3 + (_dungeonInfo[floor, index1 + 1] - 1 - index5 << 4) + 792;
      ...
      for (int index7 = _dungeonInfo[floor, index1]; index7 <= _dungeonInfo[floor, index1 + 2]; ++index7)
      {
        ...
        switch (num10)
        {
          ...
          default:
            if (GenerateRandom(0U, 1U) != 0U)
            {
              int random = (int) GenerateRandom(0U, (uint) num4);
              for (int index8 = 0; index8 < random; ++index8)
              {
                if (_dungeonInfo[floor, index7 + (_dungeonInfo[floor, index1 + 3] + index8 + 1 << 4) + 792] != 8)
                  _dungeonInfo[floor, index7 + (_dungeonInfo[floor, index1 + 3] + index8 + 1 << 4) + 792] = 0;
              }
              continue;
            }
            if (num10 != 8)
            {
              switch (_dungeonInfo[floor, index7 + (_dungeonInfo[floor, index1 + 3] + 1 << 4) + 792])
              {
                ...
                case 8:
                  continue;
                default:
                  int random3 = (int) GenerateRandom(0U, (uint) num2);
                  int num11 = index7 - 1;
                  for (int index9 = 0; index9 < random3 && _dungeonInfo[floor, index7 + (_dungeonInfo[floor, index1 + 3] - index9 << 4) + 792] != 8 && _dungeonInfo[floor, index7 + (_dungeonInfo[floor, index1 + 3] - index9 - 1 << 4) + 792] != 1 && (_dungeonInfo[floor, (_dungeonInfo[floor, index1 + 3] - index9 << 4) + index7 + 1 + 792] == 1 || _dungeonInfo[floor, (_dungeonInfo[floor, index1 + 3] - index9 - 1 << 4) + index7 + 1 + 792] != 1) && (_dungeonInfo[floor, num11 + (_dungeonInfo[floor, index1 + 3] - index9 << 4) + 792] == 1 || _dungeonInfo[floor, num11 + (_dungeonInfo[floor, index1 + 3] - index9 - 1 << 4) + 792] != 1); ++index9)
                    _dungeonInfo[floor, index7 + (_dungeonInfo[floor, index1 + 3] - index9 << 4) + 792] = 1;
                  continue;
              }
            }
      ...
      for (int index10 = _dungeonInfo[floor, index1 + 1]; index10 <= _dungeonInfo[floor, index1 + 3]; ++index10)
      {
        ...
        switch (num12)
        {
          ...
          default:
            if (GenerateRandom(0U, 1U) != 0U)
            {
              int random = (int) GenerateRandom(0U, (uint) num5);
              for (int index11 = 0; index11 < random; ++index11)
              {
                if (_dungeonInfo[floor, (index10 << 4) + 792 + _dungeonInfo[floor, index1] - 1 - index11] != 8)
                  _dungeonInfo[floor, (index10 << 4) + 792 + _dungeonInfo[floor, index1] - 1 - index11] = 0;
              }
              continue;
            }
            if (num12 != 8)
            {
              switch (_dungeonInfo[floor, (index10 << 4) + 792 + _dungeonInfo[floor, index1] - 1])
              {
                ...
                case 8:
                  continue;
                default:
                  int random4 = (int) GenerateRandom(0U, (uint) num1);
                  int num13 = index10 - 1;
                  for (int index12 = 0; index12 < random4 && _dungeonInfo[floor, index12 + (index10 << 4) + _dungeonInfo[floor, index1] + 792] != 8 && _dungeonInfo[floor, index12 + (index10 << 4) + _dungeonInfo[floor, index1] + 792 + 1] != 1 && (_dungeonInfo[floor, index12 + (index10 + 1 << 4) + _dungeonInfo[floor, index1] + 792] == 1 || _dungeonInfo[floor, index12 + (index10 + 1 << 4) + _dungeonInfo[floor, index1] + 792 + 1] != 1) && (_dungeonInfo[floor, index12 + (num13 << 4) + _dungeonInfo[floor, index1] + 792] == 1 || _dungeonInfo[floor, index12 + (num13 << 4) + _dungeonInfo[floor, index1] + 792 + 1] != 1); ++index12)
                    _dungeonInfo[floor, index12 + (index10 << 4) + _dungeonInfo[floor, index1] + 792] = 1;
                  continue;
              }
            }
      ...
      for (int index13 = _dungeonInfo[floor, index1 + 1]; index13 <= _dungeonInfo[floor, index1 + 3]; ++index13)
      {
        ...
        switch (num14)
        {
          ...
          default:
            if (GenerateRandom(0U, 1U) != 0U)
            {
              int random = (int) GenerateRandom(0U, (uint) num6);
              for (int index14 = 0; index14 < random; ++index14)
              {
                if (_dungeonInfo[floor, index14 + (index13 << 4) + _dungeonInfo[floor, index1 + 2] + 792 + 1] != 8)
                  _dungeonInfo[floor, index14 + (index13 << 4) + _dungeonInfo[floor, index1 + 2] + 792 + 1] = 0;
              }
              continue;
            }
            if (num14 != 8)
            {
              switch (_dungeonInfo[floor, _dungeonInfo[floor, index1 + 1] + (index13 << 4) + 792 + 1])
              {
                ...
                case 8:
                  continue;
                default:
                  int random5 = (int) GenerateRandom(0U, (uint) num1);
                  int num15 = index13 - 1;
                  for (int index15 = 0; index15 < random5 && _dungeonInfo[floor, (index13 << 4) + 792 + _dungeonInfo[floor, index1 + 2] - index15] != 8 && _dungeonInfo[floor, (index13 << 4) + 792 + _dungeonInfo[floor, index1 + 2] - index15 - 1] != 1 && (_dungeonInfo[floor, (index13 + 1 << 4) + 792 + _dungeonInfo[floor, index1 + 2] - index15] == 1 || _dungeonInfo[floor, (index13 + 1 << 4) + 792 + _dungeonInfo[floor, index1 + 2] - index15 - 1] != 1) && (_dungeonInfo[floor, (num15 << 4) + 792 + _dungeonInfo[floor, index1 + 2] - index15] == 1 || _dungeonInfo[floor, (num15 << 4) + 792 + _dungeonInfo[floor, index1 + 2] - index15 - 1] != 1); ++index15)
                    _dungeonInfo[floor, (index13 << 4) + 792 + _dungeonInfo[floor, index1 + 2] - index15] = 1;
                  continue;
              }
            }
    ...
    {
      int num1 = _dungeonInfo[floor, address + 3U] + 1 - _dungeonInfo[floor, address + 1U];
      if (num1 < 7 || _dungeonInfo[floor, address + 4U] != 0)
        return false;
      int random = (int) GenerateRandom(0U, (uint) (num1 - 7));
      int num2 = _dungeonInfo[floor, address + 1U] + random + 3;
      ...
      SetValue(floor, value2, _dungeonInfo[floor, address], (byte) num2, _dungeonInfo[floor, address + 2U], (byte) num2);
      SetValue(floor, value1, _dungeonInfo[floor, address], (byte) (num2 + 1), _dungeonInfo[floor, address + 2U], _dungeonInfo[floor, address + 3U]);
    ...
    {
      if (_dungeonInfo[floor, address1 + 2U] + 1 - _dungeonInfo[floor, address1] < 3 || _dungeonInfo[floor, address1 + 3U] + 1 - _dungeonInfo[floor, address1 + 1U] < 3)
        return false;
    ...
    private bool routineD(int floor, uint address)
    {
      ...
      if (num2 - num1 + 1 < 5)
      {
        ...
        _dungeonInfo[floor, address + 16U] = (byte) GenerateRandom(_dungeonInfo[floor, address], _dungeonInfo[floor, address + 2U]);
        _dungeonInfo[floor, address + 17U] = _dungeonInfo[floor, address + 3U];
        int index1 = (_dungeonInfo[floor, address + 13U] << 4) + 792 + _dungeonInfo[floor, address + 12U];
        _dungeonInfo[floor, index1] = 8;
        int index2 = (_dungeonInfo[floor, address + 17U] << 4) + 792 + _dungeonInfo[floor, address + 16U];
      ...
      else
      {
        ...
        _dungeonInfo[floor, address + 13U] = _dungeonInfo[floor, address + 1U];
        _dungeonInfo[floor, address + 14U] = (byte) GenerateRandom((uint) (num3 + 1), _dungeonInfo[floor, address + 2U]);
        ...
        _dungeonInfo[floor, address + 17U] = _dungeonInfo[floor, address + 3U];
        _dungeonInfo[floor, address + 18U] = (byte) GenerateRandom((uint) (num3 + 1), _dungeonInfo[floor, address + 2U]);
        _dungeonInfo[floor, address + 19U] = _dungeonInfo[floor, address + 3U];
        int index3 = (_dungeonInfo[floor, address + 13U] << 4) + 792 + _dungeonInfo[floor, address + 12U];
        _dungeonInfo[floor, index3] = 8;
        int index4 = (_dungeonInfo[floor, address + 15U] << 4) + 792 + _dungeonInfo[floor, address + 14U];
        _dungeonInfo[floor, index4] = 8;
        int index5 = (_dungeonInfo[floor, address + 17U] << 4) + 792 + _dungeonInfo[floor, address + 16U];
        _dungeonInfo[floor, index5] = 8;
        int index6 = (_dungeonInfo[floor, address + 19U] << 4) + 792 + _dungeonInfo[floor, address + 18U];
      ...
      if (_dungeonInfo[floor, address + 3U] - _dungeonInfo[floor, address + 1U] + 1 < 5)
      {
        ...
        _dungeonInfo[floor, address + 5U] = (byte) GenerateRandom(_dungeonInfo[floor, address + 1U], _dungeonInfo[floor, address + 3U]);
        _dungeonInfo[floor, address + 8U] = _dungeonInfo[floor, address + 2U];
        _dungeonInfo[floor, address + 9U] = (byte) GenerateRandom(_dungeonInfo[floor, address + 1U], _dungeonInfo[floor, address + 3U]);
        int index7 = (_dungeonInfo[floor, address + 5U] << 4) + 792 + _dungeonInfo[floor, address + 4U];
        _dungeonInfo[floor, index7] = 8;
        int index8 = (_dungeonInfo[floor, address + 9U] << 4) + 792 + _dungeonInfo[floor, address + 8U];
        _dungeonInfo[floor, index8] = 8;
      }
      else
      {
        int num4 = _dungeonInfo[floor, address + 1U];
        int num5 = num4 + (_dungeonInfo[floor, address + 3U] + 1 - num4) / 2 - 1;
        _dungeonInfo[floor, address + 4U] = _dungeonInfo[floor, address];
        ...
        _dungeonInfo[floor, address + 7U] = (byte) GenerateRandom((uint) (num5 + 1), _dungeonInfo[floor, address + 3U]);
        _dungeonInfo[floor, address + 8U] = _dungeonInfo[floor, address + 2U];
        ...
        _dungeonInfo[floor, address + 11U] = (byte) GenerateRandom((uint) (num5 + 1), _dungeonInfo[floor, address + 3U]);
        int index9 = (_dungeonInfo[floor, address + 5U] << 4) + 792 + _dungeonInfo[floor, address + 4U];
        _dungeonInfo[floor, index9] = 8;
        int index10 = (_dungeonInfo[floor, address + 7U] << 4) + 792 + _dungeonInfo[floor, address + 6U];
        _dungeonInfo[floor, index10] = 8;
        int index11 = (_dungeonInfo[floor, address + 9U] << 4) + 792 + _dungeonInfo[floor, address + 8U];
        _dungeonInfo[floor, index11] = 8;
        int index12 = (_dungeonInfo[floor, address + 11U] << 4) + 792 + _dungeonInfo[floor, address + 10U];
    ...
    {
      int num1 = _dungeonInfo[floor, address + 2U] + 1 - _dungeonInfo[floor, address];
      if (num1 < 7 || _dungeonInfo[floor, address + 5U] != 0)
        return false;
      int random = (int) GenerateRandom(0U, (uint) (num1 - 7));
      int num2 = _dungeonInfo[floor, address] + random + 3;
      ...
      SetValue(floor, value2, (byte) num2, _dungeonInfo[floor, address + 1U], (byte) num2, _dungeonInfo[floor, address + 3U]);
      SetValue(floor, value1, (byte) (num2 + 1), _dungeonInfo[floor, address + 1U], _dungeonInfo[floor, address + 2U], _dungeonInfo[floor, address + 3U]);
      _dungeonInfo[floor, value1 + 4U] = 0;
      _dungeonInfo[floor, value1 + 5U] = 0;
      ...
      uint value3)
    {
      ...
      {
        for (int index2 = num2 + 1; index2 < _dungeonInfo[floor, index1 + 1] + 1; ++index2)
          _dungeonInfo[floor, (index2 << 4) + 792 + num1] = 2;
        for (int index3 = num4 - 1; index3 > _dungeonInfo[floor, index1 + 1]; --index3)
...
label_42:
      ...
      {
        for (int index14 = num1 + 1; index14 < _dungeonInfo[floor, index1] + 1; ++index14)
          _dungeonInfo[floor, index14 + (num2 << 4) + 792] = 2;
        for (int index15 = num3 - 1; index15 > _dungeonInfo[floor, index1]; --index15)
...
label_95:
    ...
    private void routineJ(int floor)
    {
      ...
      while (true)
      {
        int random3 = (int) GenerateRandom(0U, _dungeonInfo[floor, 23] - 1U);
        int index1 = random3 * 20 + 472;
        int random4 = (int) GenerateRandom(_dungeonInfo[floor, index1], _dungeonInfo[floor, index1 + 2]);
        int random5 = (int) GenerateRandom(_dungeonInfo[floor, index1 + 1], _dungeonInfo[floor, index1 + 3]);
        if (num2 < 100)
        {
        ...
        do
        {
          int random6 = (int) GenerateRandom(0U, _dungeonInfo[floor, 23] - 1U);
    ...
    {
      int random1 = (int) GenerateRandom(1U, 3U);
      _dungeonInfo[floor, 8] = (byte) random1;
      int num1 = 0;
      int num2 = 0;
      do
      {
        int index = (int) GenerateRandom(0U, _dungeonInfo[floor, 23] - 1U) * 20 + 472;
        int random2 = (int) GenerateRandom(_dungeonInfo[floor, index], _dungeonInfo[floor, index + 2]);
        int random3 = (int) GenerateRandom(_dungeonInfo[floor, index + 1], _dungeonInfo[floor, index + 3]);
    ...
    {
      for (int index1 = 1; index1 < _details[1] + 1; ++index1)
      {
        int floor = index1 - 1;
        ...
        if (!createSearchData || index1 > 2)
        {
          ...
          routineF(floor, 24U);
          for (int index2 = 0; index2 < _dungeonInfo[floor, 21]; ++index2)
          {
            if (routineC(floor, (uint) (index2 * 12 + 24), (uint) (_dungeonInfo[floor, 23] * 20 + 472)))
              ++_dungeonInfo[floor, 23];
          }
          for (int index3 = 0; index3 < _dungeonInfo[floor, 21]; ++index3)
            GenerateFloorMap(floor, (uint) (index3 * 12 + 24));
          for (int index4 = 0; index4 < _dungeonInfo[floor, 22]; ++index4)
            routineG(floor, (uint) ((index4 << 4) + 216));
          for (int index5 = 0; index5 < _dungeonInfo[floor, 2]; ++index5)
          {
            _dungeonInfo[floor, index5 + 792] = 1;
            _dungeonInfo[floor, (_dungeonInfo[floor, 3] - 1 << 4) + index5 + 792] = 1;
          }
          for (int index6 = 0; index6 < _dungeonInfo[floor, 3]; ++index6)
          {
          ...
          if (!createSearchData)
          {
            ...
            int index9 = (TreasureMapDataTable.TableJ[_details[3] - 1] * 12 + _dungeonInfo[floor, 1304] - 1) * 18;
            ...
            fixed (byte* numPtr1 = &_dungeonInfo[floor, 0])
            {
              *(int*) (numPtr1 + 1308) = (numPtr1[1306] << 4) + 4896;
              int num1 = 4128 - ((numPtr1[1306] << 4) + numPtr1[1307] * 8);
              if (num1 < 0)
              {
              ...
              *(int*) numPtr6 = (int) *(uint*) numPtr6 + numPtr1[1305] * 20;
              for (int index10 = 0; index10 < numPtr1[1305]; ++index10)
              {
                fixed (byte* numPtr7 = &TreasureMapDataTable.TableK[0])
                {
                  ...
                  fixed (byte* numPtr8 = &TreasureMapDataTable.TableM[num2 * 2])
                  {
                    ...
                    *(int*) numPtr9 = (int) *(uint*) numPtr9 + (TreasureMapDataTable.TableL[*(ushort*) numPtr8] + 8);
                  }
                }
              }
              int num3 = 11216;
              int num4 = num3 < *(uint*) (numPtr1 + 1308) ? 0 : (int) (num3 - *(uint*) (numPtr1 + 1308)) / 20;
              numPtr1[1313] = 0;
              int num5 = 0;
              for (int index11 = 0; index11 < numPtr1[1305]; ++index11)
              {
      ...
      for (int index12 = 2; index12 < _details[1]; ++index12)
      {
        _seed = (uint) (MapSeed + index12 + 1);
        for (int index13 = 0; index13 < _dungeonInfo[index12, 8] << 1; ++index13)
        {
          int random = (int) GenerateRandom();
        }
        int num = _details[2] + index12 / 4;
        for (int index14 = 0; index14 < _dungeonInfo[index12, 8]; ++index14)
        {
          _dungeonInfo[index12, index14 + 9] = (byte) GetItemRank(TreasureMapDataTable.TableN[(num - 1) * 4 + 1], TreasureMapDataTable.TableN[(num - 1) * 4 + 2]);
          ++_details2[_dungeonInfo[index12, index14 + 9] - 1];
        }
        if (_dungeonInfo[index12, 8] > 0)
        {
          for (int index15 = 0; index15 < _dungeonInfo[index12, 8]; ++index15)
          {
            TreasureBoxInfo treasureBoxInfo = new TreasureBoxInfo(index15, _dungeonInfo[index12, 9 + index15], _dungeonInfo[index12, index15 * 2 + 13], _dungeonInfo[index12, index15 * 2 + 14]);
            int index16 = 0;
            while (index16 < _treasureBoxInfoList[index12].Count && treasureBoxInfo.Rank <= _treasureBoxInfoList[index12][index16].Rank)
              ++index16;
            _treasureBoxInfoList[index12].Insert(index16, treasureBoxInfo);
          }
        }
      }
    }
    ...
    public string GetTreasureBoxItem(int floor, int boxIndex, int second)
    {
      ...
      for (int index1 = 0; index1 < _dungeonInfo[floor, 8]; ++index1)
      {
        int num1 = (int) routineRandom(100U);
        if (index1 == boxIndex)
        {
    ...
    public void CalculateDetail() => CalculateDetail(false, false);

    public void CalculateDetail(bool floorDetail) => CalculateDetail(floorDetail, false);
    ...
    public void CalculateDetail(bool floorDetail, bool createSearchData)
    {
      ...
      for (int index = 0; index < 16; ++index)
        _treasureBoxInfoList[index].Clear();
      ...
      {
        int random = (int) GenerateRandom(100U);
      }
      ...
      for (int index = 0; index < 12; ++index)
        _details[index + 1 + 7] = (byte) Seek3(TreasureMapDataTable.TableF[index * 4 + 1], TreasureMapDataTable.TableF[index * 4 + 2]);
      ...
      int num1 = (_details[0] + _details[1] + _details[2] - 4) * 3 + ((int) GenerateRandom(11U) - 5);
      if (num1 < 1)
        num1 = 1;
      if (num1 > 99)
        num1 = 99;
      _details[4] = (byte) num1;
      _name3Index = TreasureMapDataTable.TreasureMapName3_IndexTable[(_details[7] - 1) * 5 + _details[3] - 1];
      if (!createSearchData)
      {
        ReadOnlyCollection<ushort> reverseSeedTable = TreasureMapDataTable.GetReverseSeedTable(MapSeed);
        if (reverseSeedTable != null)
        {
          ...
          {
            _seed = num2;
            uint random1 = GenerateRandom();
            foreach (byte num3 in _candidateRank)
            {
              int num4 = (int) (num3 + random1 % (num3 / 10 * 2 + 1) - num3 / 10);
              if (num4 > 248)
                num4 = 248;
            ...
            GenerateRandom();
            uint random2 = GenerateRandom();
            uint num5 = MapRank > 50 ? (MapRank > 80 ? random2 % 150U + 1U : random2 % 131U + 1U) : random2 % 47U + 1U;
      ...
      if (!floorDetail)
        return;
      for (int index = 1; index < _details[1] + 1; ++index)
      {
        int num9 = index > 4 ? (index > 8 ? (index > 12 ? 16 : (MapSeed + index) % 3 + 14) : (MapSeed + index) % 4 + 12) : (MapSeed + index) % 5 + 10;
    ...
    {
      get => TreasureMapDataTable.TreasureMapMapTypeName_Table[_details[3] - 1];
    }

    public byte MapTypeIndex => (byte) (_details[3] - 1U);

    public List<TreasureBoxInfo>[] TreasureBoxInfoList => _treasureBoxInfoList;
    ...
    {
      return floor < FloorCount && x < GetFloorWidth(floor) && y < GetFloorHeight(floor) && _dungeonInfo[floor, 4] == x && _dungeonInfo[floor, 5] == y;
    }
    ...
    public int IsTreasureBoxRank(int floor, int x, int y)
    {
      ...
      {
        for (int index = 0; index < GetTreasureBoxCountPerFloor(floor); ++index)
        {
    ...
    public int GetTreasureBoxIndex(int floor, int x, int y)
    {
      if (floor < FloorCount && x < GetFloorWidth(floor) && y < GetFloorHeight(floor))
      {
        for (int treasureBoxIndex = 0; treasureBoxIndex < GetTreasureBoxCountPerFloor(floor); ++treasureBoxIndex)
        {
          if (_dungeonInfo[floor, treasureBoxIndex * 2 + 13] == x && _dungeonInfo[floor, treasureBoxIndex * 2 + 14] == y)
