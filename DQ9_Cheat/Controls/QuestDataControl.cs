...
namespace DQ9_Cheat.Controls
{
  public class QuestDataControl : DataControlBase
  {
    ...
    private void QuestDateTimeNumericUpDown_ValueChanged(object sender, EventArgs e)
    {
      ...
      //   return;
      if (_updateCount != 0 || !(sender is VisionNumericUpDown visionNumericUpDown) || !(visionNumericUpDown.Tag is int[] tag))
        return;
      if (tag.Length != 2)
        return;
      int num = tag[1];
      int index = tag[0] + _vScrollBar.Value;
      switch (num)
      {
        case 0:
    ...
    private void QuestStateComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
      if (_updateCount != 0 || !(sender is VisionComboBox visionComboBox))
        return;
      int index = (int) visionComboBox.Tag + _vScrollBar.Value;
    ...
    private void panel_Paint() => panel.panel_Paint();
    ...
    protected override void OnValueUpdate()
    {
      ...
      for (int index3 = 0; index3 < 12; ++index3)
      {
        if (QuestDataList.List.Count > index2)
        {
          ...
          _questStateComboBox[index1].SelectedIndex = !questElement.AdditionalQuest ? (int) (state - 1) : (int) state;
    ...
    private void QuestDataControl_Click(object sender, EventArgs e) => textBox_Dummy.Focus();
    ...
    private void radioButton_NotRecieved_CheckedChanged(object sender, EventArgs e)
    {
      ...
      else
      {
        ...
        if ((checkBox_LumpEditAdditional.Checked || checkBox_LumpEditNormal.Checked) && (radioButton_BeforeOrder.Checked || radioButton_Cleared.Checked))
          button_Execute.Enabled = true;
        else
          button_Execute.Enabled = false;
      }
