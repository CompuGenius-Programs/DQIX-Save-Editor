...
{
    private readonly Button[] _buttonItemSelects = new Button[6];
    private readonly SafeNumericUpDown[] _numericUpDownItemCounts = new SafeNumericUpDown[6];
    private readonly SafeNumericUpDown[] _numericUpDownItemPrices = new SafeNumericUpDown[6];
    private readonly TextBox[] _textBoxItemNames = new TextBox[6];
    ...
    private void NumericUpDownItemPrices_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0 || !(sender is SafeNumericUpDown safeNumericUpDown))
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.WifiShopGoods[(int)safeNumericUpDown.Tag].ItemPrice =
            (uint)safeNumericUpDown.Value;
    }
    ...
    private void NumericUpDownItemCounts_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0 || !(sender is SafeNumericUpDown safeNumericUpDown))
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.WifiShopGoods[(int)safeNumericUpDown.Tag].ItemCount =
            (byte)safeNumericUpDown.Value;
    }
    ...
    {
        BeginUpdate();
        var wifiShopData = SaveDataManager.Instance.SaveData.WifiShopData;
        ...
        for (var index = 0; index < 6; ++index)
        {
            var itemDataBase = wifiShopData.WifiShopGoods[index].Item;
    ...
    private void numericUpDown_AllowNextConnectYear_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0)
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.AllowNextConnectYear =
            (ushort)numericUpDown_AllowNextConnectYear.Value;
    }
    ...
    private void numericUpDown_AllowNextConnectMonth_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0)
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.AllowNextConnectMonth =
            (byte)numericUpDown_AllowNextConnectMonth.Value;
    }
    ...
    private void numericUpDown_AllowNextConnectDay_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0)
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.AllowNextConnectDay =
            (byte)numericUpDown_AllowNextConnectDay.Value;
    }
    ...
    private void numericUpDown_AllowNextConnectHour_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0)
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.AllowNextConnectHour =
            (byte)numericUpDown_AllowNextConnectHour.Value;
    }

    private void textBox_Message_TextChanged(object sender, EventArgs e)
    {
        var encoding = Encoding.GetEncoding("utf-8");
        var bytes = encoding.GetBytes(textBox_Message.Text);
        if (bytes.Length > SaveDataManager.Instance.SaveData.WifiShopData.MessageMaxLength)
        {
            textBox_Message.Text =
                encoding.GetString(bytes, 0, SaveDataManager.Instance.SaveData.WifiShopData.MessageMaxLength);
    ...
    private void numericUpDown_MessageExpiresYear_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0)
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.MessageExpiresYear =
            (ushort)numericUpDown_MessageExpiresYear.Value;
    }
    ...
    private void numericUpDown_MessageExpiresMonth_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0)
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.MessageExpiresMonth =
            (byte)numericUpDown_MessageExpiresMonth.Value;
    }
    ...
    private void numericUpDown_MessageExpiresHour_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0)
            return;
        SaveDataManager.Instance.SaveData.WifiShopData.MessageExpiresHour =
            (byte)numericUpDown_MessageExpiresHour.Value;
    }
