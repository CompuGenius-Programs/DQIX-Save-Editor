...
public class AlchemyDataControl : DataControlBase
{
    ...
    {
        var alchemyData = SaveDataManager.Instance.SaveData.AlchemyData;
        var num1 = 0;
        ...
        var num2 = (int)(alchemyData.CompCount / 448.0 * 100.0);
    ...
    private void panel_MouseDown(object sender, MouseEventArgs e)
    {
        ...
        SaveDataManager.Instance.SaveData.AlchemyData.SetAlchemyState(_itemList[index2].AlchemyIndex,
            _checkBox_Checked[index1] ? AlchemyState.Created : AlchemyState.NotAppear);
        RenewalAlchemyCount();
    ...
    private void button_AllCheck_Click(object sender, EventArgs e)
    {
        ...
        for (var index = 0; index < _itemList.Length; ++index)
            SaveDataManager.Instance.SaveData.AlchemyData.SetAlchemyState(_itemList[index].AlchemyIndex,
                AlchemyState.Created);
        SaveDataManager.Instance.UndoRedoMgr.EndPluralEdit();
        OnValueUpdate();
    }
    ...
    private void comboBox_SelectType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        _itemList = selectedIndex != 0
            ? selectedIndex >= 13
                ? selectedIndex >= 20
                    ? ItemDataList.GetAlchemyList(ItemType.Tool)
                    : ItemDataList.GetAlchemyList((ItemType)(selectedIndex - 10))
                : ItemDataList.GetAlchemyWeaponList((WeaponType)(selectedIndex - 1))
            : ItemDataList.GetAllAlchemyList();
        RenewalScrollBar();
        if (_updateCount != 0)
            return;
        SetAlchemyData();
