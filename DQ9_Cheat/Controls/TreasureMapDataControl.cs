...
public class TreasureMapDataControl : DataControlBase
{
    ...
    {
        var selectedIndex = listBox_TreasureMap.SelectedIndex;
        listBox_TreasureMap.BeginUpdate();
        listBox_TreasureMap.Items.Clear();
        var treasureMapManager = SaveDataManager.Instance.SaveData.TreasureMapManager;
        for (var index = 0; index < treasureMapManager.MapCount; ++index)
        {
            var mapData = treasureMapManager.GetMapData(index);
    ...
    private bool AddListBox(int index, TreasureMapData mapData)
    {
        ...
        {
            var num = (int)numericUpDown_NarrowingLevel.Value;
        ...
        {
            if (groupBoxWithCheckBox_Narrowing.Checked && (!CheckBox_NarrowingNormal.Checked ||
                                                           (!string.IsNullOrEmpty(comboBox_NarrowingName1.SelectedItem
                                                                .ToString()) &&
                                                            comboBox_NarrowingName1.SelectedItem.ToString() !=
                                                            mapData.MapName1) ||
                                                           (!string.IsNullOrEmpty(comboBox_NarrowingName2.SelectedItem
                                                                .ToString()) &&
                                                            comboBox_NarrowingName2.SelectedItem.ToString() !=
                                                            mapData.MapName2) ||
                                                           (!string.IsNullOrEmpty(comboBox_NarrowingName3.SelectedItem
                                                                .ToString()) &&
                                                            comboBox_NarrowingName3.SelectedItem.ToString() !=
                                                            mapData.MapName3)))
                return false;
        ...
        else if (mapData.MapType == MapType.devil)
        {
            if (groupBoxWithCheckBox_Narrowing.Checked && !CheckBox_NarrowingDevil.Checked)
                return false;
            name = mapData.DevilType == null
                ? "Devil"
                : string.Format("{0}'s Map Lv. {1}", mapData.DevilType.Name, mapData.DevilLevel.Value);
        }
        else
        {
            name = "Unknown";
        ...
        var treasureMapListBoxItem1 = new TreasureMapListBoxItem(index, name);
        if (toolButton_SortLVUp.Checked || toolButton_SortLVDown.Checked)
        {
            var treasureMapManager = SaveDataManager.Instance.SaveData.TreasureMapManager;
            for (var index1 = 0; index1 < listBox_TreasureMap.Items.Count; ++index1)
                if (listBox_TreasureMap.Items[index1] is TreasureMapListBoxItem treasureMapListBoxItem2)
                {
                    var mapData1 = treasureMapManager.GetMapData(treasureMapListBoxItem2.Index);
                    var mapData2 = treasureMapManager.GetMapData(index);
                    if (toolButton_SortLVUp.Checked)
                    {
                        if (mapData1.MapType == MapType.devil && mapData2.MapType == MapType.Normal &&
                            mapData1.MapLevel >= mapData2.MapLevel)
                        {
    ...
    private void RenewalCandidatePlace()
    {
        if (_selectedMapData != null)
        {
            if (_selectedMapData.MapType == MapType.Normal)
            {
                ...
                var validPlaceList = _selectedMapData.ValidPlaceList;
    ...
    private void RenewalDungeonDetail(TreasureMapData mapData)
    {
        if (mapData != null && mapData.MapType == MapType.Normal && mapData.Rank >= 2 && mapData.Rank <= 248)
        {
            ...
            var treasureBoxCount = mapData.GetTreasureBoxCount(0);
            ...
            if (treasureBoxCount > 0)
            {
                ...
                for (var rank = 10; rank > 0; --rank)
                    stringBuilder.AppendFormat(" {0}:{1}", _treasureBoxRankSymbol[0, rank - 1],
                        mapData.GetTreasureBoxCount(rank));
                stringBuilder.Append("\n");
                stringBuilder.Append(" By floor:\n");
                for (var floor = 0; floor < mapData.FloorCount; ++floor)
                    if (mapData.GetTreasureBoxCountPerFloor(floor) > 0)
                    {
                        stringBuilder.AppendFormat("   B{0:D2}:", floor + 1);
                        foreach (var treasureBoxInfo in mapData.TreasureBoxInfoList[floor])
            ...
            for (var floor = 0; floor < mapData.FloorCount; ++floor)
            {
                ...
                var floorMap = mapData.GetFloorMap(floor);
                if (floorMap != null)
                {
                    stringBuilder.Append("\n");
                    stringBuilder.AppendFormat("B{0:D2}\n", floor + 1);
                    for (var y = 0; y < mapData.GetFloorHeight(floor); ++y)
                    {
                        for (var x = 0; x < mapData.GetFloorWidth(floor); ++x)
                            ...
                            {
                                var num = mapData.IsTreasureBoxRank(floor, x, y);
                                if (num > 0)
                                {
                                    var treasureBoxIndex = mapData.GetTreasureBoxIndex(floor, x, y);
                    ...
                    {
                        var rank = mapData.TreasureBoxInfoList[floor][index1].Rank;
                        var index2 = mapData.TreasureBoxInfoList[floor][index1].Index;
                        var treasureBoxItem = mapData.GetTreasureBoxItem(floor, index2, 2);
                        stringBuilder.AppendFormat("({0:D2}, {1:D2}) {2} {3}\n",
                            mapData.TreasureBoxInfoList[floor][index1].X, mapData.TreasureBoxInfoList[floor][index1].Y,
            ...
            using (var font = new Font("MS Gothic", 9f, FontStyle.Underline, GraphicsUnit.Point, 128))
            {
                for (var index3 = 0; index3 < mapData.FloorCount; ++index3)
    ...
    private void toolButton_CreateMap_Click(object sender, EventArgs e)
    {
        ...
        EndUpdate();
        listBox_TreasureMap.BeginUpdate();
        var mapData = treasureMapManager.GetMapData(treasureMapManager.MapCount - 1);
        RenewalListBox();
    ...
    {
        if (listBox_TreasureMap.SelectedIndex == -1 ||
            !(listBox_TreasureMap.SelectedItem is TreasureMapListBoxItem selectedItem1))
            return;
        ...
        if (listBox_TreasureMap.Items.Count > selectedIndex)
        {
            listBox_TreasureMap.SelectedIndex = selectedIndex;
            var treasureMapManager = SaveDataManager.Instance.SaveData.TreasureMapManager;
            _selectedMapData = !(listBox_TreasureMap.SelectedItem is TreasureMapListBoxItem selectedItem2)
                ? null
                : treasureMapManager.GetMapData(selectedItem2.Index);
        }
        else
        {
    ...
    {
        if (_updateCount != 0 || comboBox_Devil.SelectedIndex == -1 ||
            !(comboBox_Devil.SelectedItem is Devil selectedItem) || _selectedMapData == null)
            return;
    ...
    private void textBox_DungeonDetail_MouseMove(object sender, MouseEventArgs e)
    {
        ...
        for (var index1 = 0; index1 < _selectedMapData.FloorCount; ++index1)
    ...
    private void textBox_DungeonDetail_MouseDown(object sender, MouseEventArgs e)
    {
        if (_selectedMapData == null || _selectedMapData.MapType != MapType.Normal)
            return;
        var indexFromPosition = textBox_DungeonDetail.GetCharIndexFromPosition(new Point(e.X - 5, e.Y));
        for (var floor = 0; floor < _selectedMapData.FloorCount; ++floor)
        for (var boxIndex = 0; boxIndex < _treasureBoxCount[floor]; ++boxIndex)
            if (_treasureBoxIndexes[floor, boxIndex] == indexFromPosition)
            {
                textBox_DungeonDetail.Cursor = Cursors.Arrow;
                using (var boxItemTableList = new TreasureBoxItemTableList(_selectedMapData, floor, boxIndex))
                {
                    foreach (var treasureBoxInfo in _selectedMapData.TreasureBoxInfoList[floor])
    ...
    {
        var componentResourceManager = new ComponentResourceManager(typeof(TreasureMapDataControl));
