...
public class MonsterDataControl : DataControlBase
{
    ...
    private readonly bool[] _checkBox_Checked = new bool[20];
    private readonly Bitmap[] _checkBoxBitmap = new Bitmap[2];
    private int _lastMouseOverIndex;
    private readonly VisionNumericUpDown[] _numericUpDown_Item1Count = new VisionNumericUpDown[20];
    private readonly VisionNumericUpDown[] _numericUpDown_Item2Count = new VisionNumericUpDown[20];
    private readonly VisionNumericUpDown[] _numericUpDown_VictoryCount = new VisionNumericUpDown[20];
            ...
            using (Brush brush1 = new SolidBrush(ForeColor))
            {
                using (Brush brush2 = new SolidBrush(SystemColors.ControlLight))
                {
                    using (Brush brush3 = new SolidBrush(SystemColors.Control))
                    {
                        ...
                        var index1 = _vScrollBar.Value * 2;
                        var index2 = 0;
                        for (var index3 = 0; index3 < 10; ++index3)
                        {
                            g.FillRectangle((index3 & 1) == 0 ? brush2 : brush3,
                                new Rectangle(0, 40 + index3 * 30, Width, 30));
                            var num = 0;
                            while (num < 2)
                            {
                                if (index1 < MonsterDataList.List.Count)
                                {
                                    var s = MonsterDataList.List[index1];
                                    g.DrawString(string.Format("{0:D3}", index1 + 1), Font, brush1, 8 + num * 310,
                                        49 + index3 * 30);
    ...
    {
        var monsterDataManager = SaveDataManager.Instance.SaveData.MonsterDataManager;
        var num = (int)(monsterDataManager.CompCount / 307.0 * 100.0);
    ...
    private void panel_MouseDown(object sender, MouseEventArgs e)
    {
        ...
        if (index1 == -1)
            return;
        var index2 = _vScrollBar.Value * 2 + index1;
    ...
    public void Initialize()
    {
        ...
        using (Graphics.FromHwnd(Handle))
        {
            var width = 20;
            var height = 20;
            for (var index4 = 0; index4 < 2; ++index4)
            {
                _checkBoxBitmap[index4] = new Bitmap(width, height);
                using (var graphics = Graphics.FromImage(_checkBoxBitmap[index4]))
                {
                    graphics.Clear(Color.Transparent);
                    if (VisualStyleRenderer.IsSupported)
                        CheckBoxRenderer.DrawCheckBox(graphics, new Point(0, 0),
                            index4 == 0 ? CheckBoxState.UncheckedNormal : CheckBoxState.CheckedNormal);
                    else if (index4 == 0)
    ...
    private void NumericUpDown_VictoryCount_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0 || !(sender is VisionNumericUpDown visionNumericUpDown))
            return;
        var index = _vScrollBar.Value * 2 + (int)visionNumericUpDown.Tag;
    ...
    private void NumericUpDown_Item1Count_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0 || !(sender is VisionNumericUpDown visionNumericUpDown))
            return;
        SaveDataManager.Instance.SaveData.MonsterDataManager.SetItemCount1(
            _vScrollBar.Value * 2 + (int)visionNumericUpDown.Tag, (byte)visionNumericUpDown.Value);
    }
    ...
    private void NumericUpDown_Item2Count_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0 || !(sender is VisionNumericUpDown visionNumericUpDown))
            return;
        SaveDataManager.Instance.SaveData.MonsterDataManager.SetItemCount2(
            _vScrollBar.Value * 2 + (int)visionNumericUpDown.Tag, (byte)visionNumericUpDown.Value);
    }
    ...
    {
        var num = (ushort)numericUpDown_LumpEditCount.Value;
        SaveDataManager.Instance.UndoRedoMgr.BeginPluralEdit();
        var monsterDataManager = SaveDataManager.Instance.SaveData.MonsterDataManager;
