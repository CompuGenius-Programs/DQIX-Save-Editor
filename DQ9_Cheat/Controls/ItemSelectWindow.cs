...
public class ItemSelectWindow : Form
{
    ...
    public ItemDataBase SelectedItem { get; private set; }
    ...
    {
        var itemDataBaseArray = textBox_SearchString.Text.Length <= 0
            ? ItemDataList.GetList(_itemType, null)
            : ItemDataList.GetList(_itemType, textBox_SearchString.Text);
    ...
    private void listBox_ItemList_MouseDoubleClick(object sender, MouseEventArgs e)
    {
        if (listBox_ItemList.IndexFromPoint(e.Location) == -1)
            return;
        SelectedItem = listBox_ItemList.SelectedItem as ItemDataBase;
