...
public class ItemDataControl : DataControlBase
{
    ...
    private readonly VisionButton[] _button_SelectItem = new VisionButton[48];
    private ItemType _itemType;
    private readonly VisionNumericUpDown[] _numericUpDown_ItemCount = new VisionNumericUpDown[48];
    private readonly VisionTextBox[] _textBox_ItemName = new VisionTextBox[48];
    ...
    private readonly int[] ItemCountEveryType;
    ...
    public void Initialize()
    {
        ...
        for (var index2 = 0; index2 < 16; ++index2)
        {
            ...
            for (var index3 = 0; index3 < 3; ++index3)
            {
                ...
                var left2 = left1 + _textBox_ItemName[index1].Width + 6;
                ...
                var left3 = left2 + _numericUpDown_ItemCount[index1].Width + 6;
    ...
    private void NumericUpDown_ItemCount_ValueChanged(object sender, EventArgs e)
    {
        if (_updateCount != 0 || !(sender is VisionNumericUpDown visionNumericUpDown))
            return;
        SaveDataManager.Instance.SaveData.ItemData
                .GetItemNum(_itemType, _vScrollBar.Value * 3 + (int)visionNumericUpDown.Tag).Value =
            (byte)visionNumericUpDown.Value;
    }
    ...
    private void Button_SelectItem_Click(object sender, EventArgs e)
    {
        if (!(sender is VisionButton visionButton))
            return;
        var itemSelectWindow = new ItemSelectWindow(_itemType);
        itemSelectWindow.Location =
            PointToScreen(new Point(panel.Left + visionButton.Right, panel.Top + visionButton.Bottom));
        if (itemSelectWindow.ShowDialog() != DialogResult.OK)
            return;
        var selectedItem = itemSelectWindow.SelectedItem;
        SaveDataManager.Instance.SaveData.ItemData.SetItemData(_itemType, _vScrollBar.Value * 3 + (int)visionButton.Tag,
            selectedItem);
        SetItemData();
    }
    ...
    private void SetItemData()
    {
        ...
        if (_vScrollBar.Value > _vScrollBar.Maximum - _vScrollBar.LargeChange)
            _vScrollBar.Value = _vScrollBar.Maximum - _vScrollBar.LargeChange;
        var index1 = _vScrollBar.Value * 3;
        var index2 = 0;
        ...
        for (var index4 = 0; index4 < 3; ++index4)
        {
            ...
            else
            {
                ...
                var itemData1 = SaveDataManager.Instance.SaveData.ItemData;
                var itemData2 = itemData1.GetItemData(_itemType, index1);
    ...
    private void ItemDataControl_Click(object sender, EventArgs e)
    {
        textBox_Dummy.Focus();
    }
    ...
    {
        var count = (byte)numericUpDown_LumpEditItemCount.Value;
    ...
    {
        if (checkBox_LumpEditAccessory.Checked || checkBox_LumpEditArm.Checked || checkBox_LumpEditBag.Checked ||
            checkBox_LumpEditHead.Checked || checkBox_LumpEditImportant.Checked || checkBox_LumpEditLowerBody.Checked ||
            checkBox_LumpEditShield.Checked || checkBox_LumpEditShoe.Checked || checkBox_LumpEditUpperBody.Checked ||
            checkBox_LumpEditWeapon.Checked)
