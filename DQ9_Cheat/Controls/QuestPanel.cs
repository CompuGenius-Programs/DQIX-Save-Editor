...
namespace DQ9_Cheat.Controls
{
  public class QuestPanel : JS_Panel
  {
    ...
    public QuestPanel() => Disposed += QuestPanel_Disposed;
    ...
    protected override void OnPaint(PaintEventArgs e) => panel_Paint(e.Graphics);

    public void panel_Paint()
    {
      using (Graphics panelGraphics = Graphics.FromHwnd(Handle))
        ...
        using (Brush brush1 = new SolidBrush(ForeColor))
        {
          using (Brush brush2 = new SolidBrush(SystemColors.ControlLight))
          {
            using (Brush brush3 = new SolidBrush(SystemColors.Control))
            {
              ...
              int questIndex = _questIndex;
              int num = 0;
              while (num < 12)
              {
                g.FillRectangle((num & 1) == 0 ? brush2 : brush3, new Rectangle(0, 20 + num * 28, Width, 28));
                if (questIndex < QuestDataList.List.Count)
                {
                  QuestElement questElement = QuestDataList.List[questIndex];
                  g.DrawString(string.Format("{0:D3}", questElement.QuestNo), Font, brush1, 6f, 28 + num * 28);
