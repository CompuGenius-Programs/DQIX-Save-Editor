...
public class AddressSelectWindow : Form
{
    ...
    public ProfileAddress SelectedAddress { get; private set; }

    private void RenewalList()
    {
        var readOnlyCollection = comboBox_SelectType.SelectedIndex != 0
            ? comboBox_SelectType.SelectedIndex != 1
                ? comboBox_SelectType.SelectedIndex != 2
                    ? ProfileAddressList.UnknownAreaList
                    : ProfileAddressList.DQAreaList
                : ProfileAddressList.OtherAreaList
            : ProfileAddressList.PrefectureList;
        Regex regex = null;
        ...
        foreach (var profileAddress in readOnlyCollection)
            if (regex != null)
            {
    ...
    private void listBox_ItemList_MouseDoubleClick(object sender, MouseEventArgs e)
    {
        if (listBox_AddressList.IndexFromPoint(e.Location) == -1)
            return;
        SelectedAddress = listBox_AddressList.SelectedItem as ProfileAddress;
